...
public class OrderServiceTest {
  ...
  public void 상품주문() throws Exception {
    ...
    Long orderId = orderService.order(
        member.getId(), book.getId(), orderCount);
    ...
    assertEquals("주문 가격 = 가격 x 수량이다", 1000 * orderCount, getOrder.getTotalPrice());
  ...
  public void 상품주문_재고수량초과() throws Exception {
    ...
    //when
    orderService.order(
        member.getId(), item.getId(), orderCount);
  ...
  private Member createMember(String name) {
    ...
    member.setName(name);
    member.setAddress(new Address("서울", "강변", "456-456"));
    em.persist(member);
    return member;
  }
}
